...
__global__ void gemm_shared_tile(const float* A, const float* B, float* C,
...
int main() {
    ...
    for (int i = 0; i < WARMUP; ++i) gemm_shared_tile<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
    ...
    for (int i = 0; i < REPEATS; ++i) gemm_shared_tile<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
