...
static void usage(
    ...
    std::cout
        ...
        << " B N K [options]\n\n"
        << "Options:\n"
        << "  --kernel auto|specialized|batch|warp|register|naive   default: auto\n"
        << "  --compare-v3              also benchmark V3 batch-select baseline\n"
        << "  --compare-v2              also benchmark V2 warp baseline\n"
        << "  --compare-v1              also benchmark V1 register baseline\n"
        ...
        << "V4 specialized path supports K in {1,2,4,8,16}; auto falls back to V3 for other K<=16.\n";
}
...
int main(
    ...
    bool compare_v3 = false;
    bool compare_v2 = false;
    bool compare_v1 = false;
    ...
    if (kernel == "auto") {
        if (topk_specialized_v4_supported(k)) {
            selected =
            ...
            implementation =
                std::string("auto->")
                +
                topk_specialized_v4_name(k);
        } else if (topk_batch_v3_supported(k)) {
            selected =
                launch_topk_batch_v3;

            implementation =
                "auto->warp_batch_threshold_bitonic_v3";
        } else {
            std::cerr
                << "Auto dispatcher currently supports 1 <= K <= 16.\n";

            return 3;
        }

    } else if (kernel == "specialized") {
        if (!topk_specialized_v4_supported(k)) {
            std::cerr
                << "V4 specialized kernel supports K in {1,2,4,8,16}.\n";
    ...
    } else if (kernel == "batch") {
        if (!topk_batch_v3_supported(k)) {
            std::cerr
                << "V3 batch-select kernel supports 1 <= K <= 16.\n";

            return 3;
        }

        selected = launch_topk_batch_v3;
        implementation =
        ...
        if (!topk_warp_v2_supported(k)) {
            std::cerr
                << "V2 warp kernel supports 1 <= K <= 16.\n";

            return 3;
        }

        selected = launch_topk_warp_v2;
        implementation =
    ...
    if (
        ...
        std::cout
            << "\nV3 comparison\n"
            << "V3 warp-batch latency = "
            << v3_ms
            << " ms\n"
            << "Selected speedup over V3 = "
            << v3_ms / latency_ms
            << "x\n";
    }
    ...
    if (
        ...
        std::cout
            ...
            << naive_ms
            << " ms\n"
            << "Selected-kernel speedup over V0 = "
            << naive_ms / latency_ms
            << "x\n";
    }
