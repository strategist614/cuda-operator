...
bool kernel_supported_on_device(const KernelConfig& kernel) {
    ...
    if (kernel.threads > prop.maxThreadsPerBlock) return false;
    if (kernel.shared_memory_bytes > prop.sharedMemPerBlock) return false;
...
TuneResult autotune_gemm(
    ...
    for (const auto& kernel : get_kernel_registry()) {
        ...
        const float latency = benchmark_kernel(
            kernel,
            A, B, C, bias,
            M, N, K,
            epilogue,
        ...
        const double perf = gemm_tflops(M, N, K, latency);

        std::cout
            << std::left << std::setw(28) << kernel.name
            << " latency = " << std::setw(10) << latency << " ms"
            << "  TFLOPS = " << perf << "\n";
    ...
    TuneResult result{
        best_kernel,
        best_latency,
        gemm_tflops(M, N, K, best_latency)
    };
