...
const std::vector<KernelConfig>& get_kernel_registry() {
    static const std::vector<KernelConfig> registry = {
        {"m16_n64_k16_t1x4",   16,  64, 16, 1, 4, 256, (16*16  + 16*64)  * (int)sizeof(float), &kernel_launcher<16,64,16,1,4>},
        {"m32_n64_k16_t2x4",   32,  64, 16, 2, 4, 256, (32*16  + 16*64)  * (int)sizeof(float), &kernel_launcher<32,64,16,2,4>},
        {"m64_n64_k16_t4x4",   64,  64, 16, 4, 4, 256, (64*16  + 16*64)  * (int)sizeof(float), &kernel_launcher<64,64,16,4,4>},
        {"m64_n128_k16_t4x8",  64, 128, 16, 4, 8, 256, (64*16  + 16*128) * (int)sizeof(float), &kernel_launcher<64,128,16,4,8>},
        {"m128_n64_k16_t8x4", 128,  64, 16, 8, 4, 256, (128*16 + 16*64)  * (int)sizeof(float), &kernel_launcher<128,64,16,8,4>},
        {"m128_n128_k8_t8x8", 128, 128,  8, 8, 8, 256, (128*8  + 8*128)  * (int)sizeof(float), &kernel_launcher<128,128,8,8,8>},
        {"m64_n16_k16_t4x1",   64,  16, 16, 4, 1, 256, (64*16  + 16*16)  * (int)sizeof(float), &kernel_launcher<64,16,16,4,1>},
        {"m64_n32_k16_t4x2",   64,  32, 16, 4, 2, 256, (64*16  + 16*32)  * (int)sizeof(float), &kernel_launcher<64,32,16,4,2>},
        {"m32_n128_k8_t2x8",   32, 128,  8, 2, 8, 256, (32*8   + 8*128)  * (int)sizeof(float), &kernel_launcher<32,128,8,2,8>}
    };
    return registry;
}

const KernelConfig* find_kernel(const std::string& name) {
    for (const auto& kernel : get_kernel_registry()) {
        if (name == kernel.name) {
            return &kernel;
        }
    }
