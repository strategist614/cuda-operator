...
bool TuneCache::lookup(
    ...
    const int epi = static_cast<int>(epilogue);
...
void TuneCache::upsert(const CacheRecord& record) {
    for (auto& r : records_) {
        if (r.gpu == record.gpu &&
...
void TuneCache::save() const {
    ...
    if (!out) {
        throw std::runtime_error("Failed to open tune cache: " + path_);
    }

    out << "gpu,cc_major,cc_minor,M,N,K,epilogue,kernel,latency_ms,tflops\n";
